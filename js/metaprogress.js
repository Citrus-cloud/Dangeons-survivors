...
const TALENT_CONFIG = {
  ...
  dexterity: {
    ...
    icon: '➤', color: '#2ecc71',
    desc: '+5% скорости, -3% кулдаунов за уровень',
    costs: [50, 150, 350, 700, 1200],
    maxLevel: 5,
  },
  intelligence: {
  ...
  constitution: {
    ...
    icon: '🛡', color: '#3498db',
    desc: '+15 макс. HP, +1 HP/5с регенерации за уровень',
    costs: [50, 150, 350, 700, 1200],
    maxLevel: 5,
  },
  charisma: {
...
const MetaProgress = {
  ...
  applyTalents(player) {
    ...
    if (t.intelligence > 0) {
      ...
      player.critChance += t.intelligence * 0.01;
    }
    ...
    // Харизма: +5% XP и золота (золото учитывается в addGold, XP — в loot)
    // XP бонус применяется через player.xpBonusMul
    if (t.charisma > 0) {
