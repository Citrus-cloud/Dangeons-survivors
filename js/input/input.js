'use strict';
/* ============================================================
   input.js — Система ввода (клавиатура + мобильный джойстик).
   
   Поддержка:
   • Клавиатура: WASD / Стрелки — движение; P / Escape — пауза
   • Динамический джойстик (мобильные):
     - pointerdown → появляется база в точке касания
     - pointermove → стик смещается (MAX_OFFSET), нормализованный вектор
     - pointerup → джойстик скрывается, движение = 0
   • Обе системы работают одновременно (приоритет: джойстик)
   
   API:
   • Input.init() — Инициализация слушателей событий
   • Input.getMove() → {x, y} — Нормализованный вектор движения
   • Input.releaseJoystick() — Принудительный сброс (при паузе)
   
   Экспорт: window.Input
   ============================================================ */
