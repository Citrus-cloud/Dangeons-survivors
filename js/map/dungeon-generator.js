'use strict';
/* ============================================================
   dungeon-generator.js — Карта, камера, наземные эффекты, 
   процедурная генерация подземелья.
   
   Содержит:
   ─────────
   • GameMap — Главный объект карты игры
   • PRNG (_mulberry32) — Детерминированный генератор псевдослучайных чисел
   • Генерация подземелья (комнаты, коридоры, колонны, факелы)
   • Наземные эффекты (лужи: slime/rot/fire)
   • Камера (следование за игроком)
   • Миникарта
   • Система биомов (визуальные стили)
   • Портал перехода между картами
   
   Процедурная генерация:
   ───────────────────────
   1. Генерация комнат (BSP / случайное размещение)
   2. Соединение коридорами (A → B без пересечений)
   3. Размещение колонн в комнатах
   4. Размещение ловушек (шипы, огонь, биомные)
   5. Размещение декора (факелы, руны, саркофаги)
   6. Создание секретной комнаты
   7. Построение сетки коллизий (grid)
   
   Ключевые методы:
   • GameMap.generateDungeon(biomeId, mapNumber) — Полная генерация
   • GameMap.moveWithCollision(x, y, dx, dy, r) — Движение с коллизиями
   • GameMap.getCamera(player, viewW, viewH) — Позиция камеры
   • GameMap.randomEnemySpawnPoint(player, minD, maxD) — Точка спавна
   • GameMap.spawnPortal() — Создать портал перехода
   • GameMap.isPlayerInPortal(player) — Проверка входа в портал
   
   Зависимости:
   CONFIG, BIOMES, BIOME_TRAP_CONFIG, PORTAL_CONFIG, INFINITE_MODE,
   Utils, ObjectPool, Particles
   
   Экспорт: window.GameMap
   ============================================================ */

/* ============================================================
   Детерминированный PRNG (Mulberry32).
   Используется при ненулевом CONFIG.DUNGEON.SEED для 
   воспроизводимой генерации карт.
   При seed = 0 используется Math.random().
   ============================================================ */
...
const GameMap = {
  /** @type {ObjectPool|null} Пул наземных эффектов (лужи/следы) */
  groundEffects: null,

  /** @type {Function} Генератор случайных чисел (Math.random или PRNG) */
  rng: Math.random,

  /** @type {Object|null} Структура текущего подземелья (rooms, corridors, grid...) */
  dungeon: null,

  /** @type {number} Игровое время для анимаций ловушек/факелов (sec) */
  time: 0,

  /** @type {HTMLCanvasElement|null} Кэш фоновых тайлов (offscreen canvas) */
  _floorCache: null,

  /** @type {Object|null} Текущий биом (объект из массива BIOMES) */
  currentBiome: null,

  /** @type {number} Номер текущей карты (1-based, для бесконечного режима) */
  currentMapNumber: 1,

  /** @type {Object|null} Портал перехода на следующую карту */
  portal: null,

  /** @type {number} Ширина текущей карты в пикселях */
  mapW: 2000,
  /** @type {number} Высота текущей карты в пикселях */
  mapH: 2000,

  /* ============================================================
     Предвычисления: инициализация пула наземных эффектов.
     Вызывается из Registry.initAll() при старте игры.
     Подземелье генерируется отдельно (generateDungeon) после старта забега.
     ============================================================ */
  precompute() {
  ...
  /**
   * Очистить все наземные эффекты (вызывается при старте нового забега/карты).
   */
  clearGroundEffects() {
  ...
  /**
   * Создать наземный эффект (лужу/след) в указанной точке.
   * 
   * @param {string} kind - Тип эффекта: 'slime' | 'rot' | 'fire' | 'acid' | 'water'
   * @param {number} x - X-координата центра
   * @param {number} y - Y-координата центра
   * @param {Object} opts - Параметры: { radius, life, slow, dps, color }
   * @returns {Object|null} Созданный эффект или null если пул исчерпан
   */
  ...
  /**
   * Запросить суммарное замедление и DPS наземных эффектов в точке.
   * Используется для применения эффектов к герою (замедление + урон).
   * 
   * @param {number} x - X-координата проверки
   * @param {number} y - Y-координата проверки
   * @returns {{slow: number, dps: number}} Макс. замедление (0..1) и суммарный DPS
   */
  queryGroundAt(x, y) {
