/* ============================================================
   pathfinding.js — A* поиск пути для умного ИИ врагов.
   
   Назначение:
   Обеспечивает навигацию врагов через подземелье по оптимальному
   маршруту (обход стен, колонн, дверей).
   
   Алгоритм:
   • A* с бинарной кучей (min-heap по fScore)
   • 8 направлений движения (ортогональные + диагональные)
   • Эвристика: манхэттенское расстояние
   • Ограничение итераций (maxNodes) для производительности
   
   Оптимизации:
   • Переиспользуемые плоские массивы (Uint8Array, Float32Array)
   • Кэш размера сетки (не пересоздаётся каждый кадр)
   • Ограничение 200-300 нод на поиск
   • Пересчёт пути не чаще 0.3-0.5 сек (управляется из enemies.js)
   
   Использует:
   • GameMap.dungeon.grid — Сетка проходимости (1 = проходимо, 0 = стена)
   • GameMap.dungeon.gridW/gridH — Размеры сетки
   • GameMap.dungeon.cellSize — Размер ячейки в пикселях
   
   Экспорт: window.Pathfinding
   ============================================================ */

const Pathfinding = {
  /** @type {number[]} Переиспользуемая бинарная куча для A* */
  _openHeap: [],
  /** @type {Uint8Array|null} Массив закрытых нод */
  _closedSet: null,
  /** @type {Float32Array|null} G-scores для каждой ноды */
  _gScores: null,
  /** @type {Int32Array|null} Родительские ноды (для восстановления пути) */
  _parents: null,

  /**
   * Найти оптимальный путь от точки (sx,sy) до (tx,ty).
   * Работает на сетке коллизий подземелья.
   * 
   * @param {number} sx - Стартовая X-координата (мировая)
   * @param {number} sy - Стартовая Y-координата (мировая)
   * @param {number} tx - Целевая X-координата (мировая)
   * @param {number} ty - Целевая Y-координата (мировая)
   * @param {number} [maxNodes=200] - Лимит итераций (для производительности)
   * @returns {Array<{x:number, y:number}>|null} Массив waypoint'ов или null
   */
