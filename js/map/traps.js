/* ============================================================
   traps.js — Наземные эффекты и загадки/ловушки (Шаг 4, 17).
   
   Содержит:
   ─────────
   • createGroundEffect() — Фабрика объекта лужи/следа для ObjectPool
   • GROUND_DEFAULT_COLOR — Цвета по умолчанию для типов эффектов
   • GROUND_OUTLINE_COLOR — Цвета обводки для типов эффектов
   • GameMap._placeStep17Objects() — Размещение загадок при генерации
   • GameMap._placeRunePuzzle() — Размещение руновой загадки
   • GameMap._placeFloorPuzzle() — Размещение загадки с плитами
   • GameMap._placeBoulders() — Размещение катящихся валунов
   • GameMap._placeVanishingPlatforms() — Размещение исчезающих платформ
   • GameMap._placeMagicFloorRunes() — Размещение магических рун на полу
   
   Типы наземных эффектов:
   ┌────────┬───────────────────────────┬───────────────────┐
   │ Тип    │ Эффект                    │ Источник          │
   ├────────┼───────────────────────────┼───────────────────┤
   │ slime  │ Замедление (slow)         │ Ooze, Spider Queen│
   │ rot    │ Замедление                │ Zombie            │
   │ fire   │ Урон в секунду (dps)      │ Fire Elem, трапы  │
   │ acid   │ DPS + замедление          │ Acid Slug         │
   │ water  │ Замедление                │ Water Elem, Ice   │
   └────────┴───────────────────────────┴───────────────────┘
   
   Зависимости: GameMap, Utils, STEP17_CONFIG, ObjectPool
   Экспорт: window.createGroundEffect
   ============================================================ */

/**
 * Фабрика наземного эффекта (лужа/след) для ObjectPool.
 * Эффекты создаются при смерти врагов, движении боссов,
 * срабатывании ловушек.
 * 
 * @returns {Object} Пустой объект эффекта с active: false
 */
...
/* --- Цвета по умолчанию для каждого типа наземного эффекта --- */
const GROUND_DEFAULT_COLOR = {
  slime: 'rgba(255, 160, 60, 0.45)',   // Оранжевая слизь
  rot:   'rgba(120, 200, 100, 0.45)',   // Зелёная гниль
  fire:  'rgba(255, 120, 30, 0.55)',    // Оранжевый огонь
};

/* --- Цвета обводки для визуального выделения эффектов --- */
const GROUND_OUTLINE_COLOR = {
  slime: 'rgba(255, 200, 130, 0.7)',    // Светло-оранжевый
  rot:   'rgba(180, 240, 160, 0.7)',    // Светло-зелёный
  fire:  'rgba(255, 220, 130, 0.9)',    // Жёлто-оранжевый
};

window.createGroundEffect = createGroundEffect;

/* ============================================================
   Шаг 17: Интеграция загадок и новых ловушек в процедурную
   генерацию подземелья.
   
   Вызывается в конце GameMap.generateDungeon() после создания
   всех комнат и коридоров. Размещает интерактивные объекты:
   • Руновые загадки (активация в правильном порядке)
   • Напольные загадки (шифр на плитах)
   • Катящиеся валуны (в длинных коридорах)
   • Исчезающие платформы (над пропастями)
   • Магические руны на полу (случайные эффекты при наступлении)
   ============================================================ */

/**
 * Разместить все объекты Шага 17 на сгенерированной карте.
 * Выбирает количество и тип объектов в зависимости от режима
 * (кампания vs бесконечный) и номера карты.
 * 
 * @param {Object} dungeon - Структура подземелья
 * @param {number} mapNumber - Номер карты (1-based)
 * @param {boolean} isCampaign - Режим кампании (фиксированный набор)
 */
